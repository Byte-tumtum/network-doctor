# Graph Report - network-doctor  (2026-07-28)

## Corpus Check
- 107 files · ~145,515 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 843 nodes · 1565 edges · 113 communities (70 shown, 43 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 266 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `15f6f1d2`
...
## Communities (113 total, 43 thin omitted)

### Community 0 - "Model Extra Tests"
Cohesion: 0.09
Nodes (81): T, TestAppendJobLine(), TestClassifyJob(), TestConcurrentToolsCanSwitch(), TestDeferredQuit(), TestDeferredRestart(), TestDeferredRestartDefersTargetSwap(), TestEscClearsCommittedFilterBeforeClosing() (+73 more)

### Community 1 - "UI Model State"
Cohesion: 0.08
Nodes (21): Context, ResolveNames(), Reader, parseSSHAliases(), SSHHostAliases(), T, TestParseSSHAliases(), CancelFunc (+13 more)

### Community 2 - "Probe Configuration Build"
Cohesion: 0.09
Nodes (42): CertPool, Config, Attempt, netops, Portal, Probe, ProbeID, ProbeResult (+34 more)
...
Cohesion: 0.07
Nodes (52): fakeConn, scriptConn, silentConn, Addr, Conn, Context, Reader, T (+44 more)
...
Cohesion: 0.11
Nodes (26): FlagSet, ExitCode(), Model, T, TestToolboxExitZero(), TestToolsFor(), init(), main() (+18 more)
...
### Community 10 - "ssid"
Cohesion: 0.06
Nodes (33): advertisedName, iwreq, F, avahiTXT(), advertisedNames(), Context, parseAvahiNames(), unescapeAvahi() (+25 more)
...
### Community 87 - "Target"
Cohesion: 0.18
Nodes (21): T, mustTarget(), TestBuildProbesNamesProtocolApplicationRow(), TestBuildProbesShape(), TestSSIDDoesNotGateNetworkProbes(), TestWrapRunTimesAndCleans(), Diagnose(), DowngradeEgress() (+13 more)
...
Cohesion: 0.09
Nodes (29): Proto, FileMode, parsePort(), ParseTarget(), TestParseTarget(), TestParseTargetCanonicalRaw(), TestParseTargetErrors(), TestProtoString() (+21 more)
...
## Knowledge Gaps
- **196 isolated node(s):** `github.com/heymaikol/network-doctor`, `advertisedName`, `iwreq`, `ToolOutputMsg`, `scheduleMsg` (+191 more)
...
- `Diagnose()` (7× useful, score=5.729527171)
- `README.md` (6× useful, score=4.425705518)
- `.handleViewKey()` (5× useful, score=3.850905128)
- `.outputView()` (5× useful, score=3.695836847)
- `Probe` (4× useful, score=3.18315952)
- `.Update()` (4× useful, score=2.976942909)
- `JSON output` (3× useful, score=2.562279995)
- `.viewerFooter()` (3× useful, score=2.439512238)
- `DowngradeEgress()` (3× useful, score=2.382863094)
- `report` (3× useful, score=2.271029443)
...
- **Why does `New()` connect `Network Connection Test Doubles` to `Model Extra Tests`, `UI Model State`, `Probe Configuration Build`, `parseSSHAliases`, `Probe Construction Tests`, `ssid`, `toolsFor`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `newModel()` connect `Model Extra Tests` to `Probe Configuration Build`, `parseSSHAliases`, `Network Connection Test Doubles`, `Probe Construction Tests`, `model_extra_test.go`, `toolsFor`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `Target` connect `Probe Configuration Build` to `Model Extra Tests`, `UI Model State`, `parseSSHAliases`, `Network Connection Test Doubles`, `toolsFor`, `Target`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Are the 37 inferred relationships involving `newModel()` (e.g. with `TestInit()` and `TestConcurrentToolsCanSwitch()`) actually correct?**
  _`newModel()` has 37 INFERRED edges - model-reasoned connections that need verification._
- **Are the 18 inferred relationships involving `asModel()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`asModel()` has 18 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 26 inferred relationships involving `New()` (e.g. with `cleanResult()` and `TestApplyDialWarnings()`) actually correct?**
  _`New()` has 26 INFERRED edges - model-reasoned connections that need verification._