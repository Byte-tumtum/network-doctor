# Graph Report - network-doctor  (2026-07-30)

## Corpus Check
- 110 files · ~172,153 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 961 nodes · 1873 edges · 119 communities (77 shown, 42 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 312 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5dd98a47`
...
1. `newModel()` - 87 edges
2. `asModel()` - 61 edges
3. `mustTarget()` - 49 edges
4. `keyMsg()` - 34 edges
5. `ProbeID` - 31 edges
6. `ProbeResult` - 31 edges
7. `netops` - 29 edges
8. `model` - 28 edges
9. `model` - 27 edges
10. `Target` - 22 edges
...
## Communities (119 total, 42 thin omitted)

### Community 0 - "Model Extra Tests"
Cohesion: 0.09
Nodes (87): T, TestAppendJobLine(), TestClassifyJob(), TestConcurrentToolsCanSwitch(), TestDeferredQuit(), TestDeferredRestart(), TestDeferredRestartDefersTargetSwap(), TestEscClearsCommittedFilterBeforeClosing() (+79 more)

### Community 1 - "UI Model State"
Cohesion: 0.19
Nodes (13): Duration, Time, jobState, loadHistory(), NewWithSource(), saveHistory(), lanNameMsg, lanNamesMsg (+5 more)

### Community 2 - "Probe Configuration Build"
Cohesion: 0.08
Nodes (57): CertPool, Config, Attempt, netops, Portal, Probe, ProbeID, ProbeResult (+49 more)
...
### Community 87 - "Target"
Cohesion: 0.15
Nodes (26): T, mustTarget(), TestBuildProbesNamesProtocolApplicationRow(), TestBuildProbesShape(), TestSSIDDoesNotGateNetworkProbes(), TestWrapRunTimesAndCleans(), Diagnose(), DowngradeEgress() (+18 more)
...
Cohesion: 0.10
Nodes (25): Cmd, KeyMsg, Model, model, listSSHKeys(), newSSHForm(), runSSH(), sshCommand() (+17 more)
...
### Community 113 - "model"
Cohesion: 0.18
Nodes (6): CancelFunc, Cmd, Context, IP, Msg, model
...
## Knowledge Gaps
- **205 isolated node(s):** `github.com/heymaikol/network-doctor`, `advertisedName`, `iwreq`, `jobState`, `ToolOutputMsg` (+200 more)
...
- `Diagnose()` (7× useful, score=5.473589605)
- `README.md` (6× useful, score=4.228009572) _(code changed — re-verify)_
- `.handleViewKey()` (5× useful, score=3.678885473)
- `.outputView()` (5× useful, score=3.530744082) _(code changed — re-verify)_
- `Probe` (4× useful, score=3.040968014)
- `.Update()` (4× useful, score=2.843963084) _(code changed — re-verify)_
- `JSON output` (3× useful, score=2.447823132) _(code changed — re-verify)_
- `.viewerFooter()` (3× useful, score=2.330539402) _(code changed — re-verify)_
- `DowngradeEgress()` (3× useful, score=2.276420772)
- `report` (3× useful, score=2.169582721)
...
- **Why does `Target` connect `parseSSHAliases` to `Model Extra Tests`, `UI Model State`, `Probe Configuration Build`, `ssh.go`, `IP`, `model`, `toolsFor`, `Target`?**
  _High betweenness centrality (0.083) - this node is a cross-community bridge._
- **Why does `newModel()` connect `Model Extra Tests` to `UI Model State`, `parseSSHAliases`, `ssh.go`, `IP`, `toolsFor`, `TestInit`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `Clean()` connect `FuzzSanitize` to `UI Model State`, `Probe Configuration Build`, `Network Connection Test Doubles`, `parseSSHAliases`, `Job Execution Lifecycle`, `ssid`, `IP`, `parseAvahiNames`, `model`, `T`?**
  _High betweenness centrality (0.064) - this node is a cross-community bridge._
- **Are the 46 inferred relationships involving `newModel()` (e.g. with `TestInit()` and `TestConcurrentToolsCanSwitch()`) actually correct?**
  _`newModel()` has 46 INFERRED edges - model-reasoned connections that need verification._
- **Are the 26 inferred relationships involving `asModel()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`asModel()` has 26 INFERRED edges - model-reasoned connections that need verification._
- **Are the 41 inferred relationships involving `mustTarget()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`mustTarget()` has 41 INFERRED edges - model-reasoned connections that need verification._
- **Are the 11 inferred relationships involving `keyMsg()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`keyMsg()` has 11 INFERRED edges - model-reasoned connections that need verification._