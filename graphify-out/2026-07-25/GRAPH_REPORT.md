# Graph Report - network-doctor  (2026-07-25)

## Corpus Check
- 89 files · ~136,995 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 739 nodes · 1420 edges · 103 communities (62 shown, 41 thin omitted)
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 227 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7e9612cd`
...
2. `model` - 64 edges
3. `asModel()` - 48 edges
4. `mustTarget()` - 41 edges
5. `keyMsg()` - 26 edges
6. `New()` - 25 edges
7. `ProbeID` - 24 edges
8. `ProbeResult` - 23 edges
9. `netops` - 23 edges
10. `toolsFor()` - 21 edges
...
Cohesion: 0.10
Nodes (73): T, TestAppendJobLine(), TestClassifyJob(), TestConcurrentToolsCanSwitch(), TestDeferredQuit(), TestDeferredRestart(), TestDeferredRestartDefersTargetSwap(), TestEscClearsCommittedFilterBeforeClosing() (+65 more)

### Community 1 - "UI Model State"
Cohesion: 0.08
Nodes (14): Context, ResolveNames(), appendJobLine(), filterLines(), CancelFunc, Cmd, Context, IP (+6 more)

### Community 2 - "Probe Configuration Build"
Cohesion: 0.11
Nodes (34): CertPool, Config, Attempt, netops, Probe, ProbeID, ProbeResult, Status (+26 more)
...
Cohesion: 0.07
Nodes (35): fakeConn, iwreq, scriptConn, silentConn, Addr, Conn, Context, Reader (+27 more)
...
Cohesion: 0.12
Nodes (23): FlagSet, ExitCode(), Model, T, TestToolboxExitZero(), TestToolsFor(), init(), main() (+15 more)
...
### Community 20 - "model"
Cohesion: 0.09
Nodes (34): Proto, Target, T, TestBannerProbeReadTimeout(), TestBannerProbeReadTimeoutHonorsContext(), TestDialIPsCancelledStopsEarly(), TestDialIPsRacesStaggered(), TestDNSProbeErrors() (+26 more)
...
Cohesion: 0.22
Nodes (15): FileMode, copyReport(), exportReport(), osc52Sequence(), captureStderr(), T, TestCopyReportWritesOSC52(), TestExportReportSavePath() (+7 more)
...
## Knowledge Gaps
- **177 isolated node(s):** `github.com/heymaikol/network-doctor`, `iwreq`, `ToolOutputMsg`, `scheduleMsg`, `lanNamesMsg` (+172 more)
...
- `Diagnose()` (6× useful, score=5.111723698)
- `.handleViewKey()` (5× useful, score=4.157971533) _(code changed — re-verify)_
- `.outputView()` (5× useful, score=3.9905383) _(code changed — re-verify)_
- `README.md` (5× useful, score=3.699355665)
- `Probe` (4× useful, score=3.436980718)
- `.Update()` (4× useful, score=3.21432065) _(code changed — re-verify)_
- `.viewerFooter()` (3× useful, score=2.634035922) _(code changed — re-verify)_
- `DowngradeEgress()` (3× useful, score=2.572869646)
- `report` (3× useful, score=2.452118518)
- `Install` (3× useful, score=1.988622431)
...
- **Why does `model` connect `UI Model State` to `Probe Configuration Build`, `model`, `UI Application Toolbox`, `Job Execution Lifecycle`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `newModel()` connect `Model Extra Tests` to `ExitCode`, `UI Application Toolbox`, `ResolveNames`, `Probe Construction Tests`, `model`?**
...
- **Are the 34 inferred relationships involving `mustTarget()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`mustTarget()` has 34 INFERRED edges - model-reasoned connections that need verification._
- **What connects `github.com/heymaikol/network-doctor`, `iwreq`, `ToolOutputMsg` to the rest of the system?**
