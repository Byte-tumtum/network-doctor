# Graph Report - network-doctor  (2026-07-31)

## Corpus Check
- 111 files · ~172,772 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 995 nodes · 1758 edges · 141 communities (81 shown, 60 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 185 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7fe00de5`
...
8. `model` - 28 edges
9. `keyMsg()` - 23 edges
10. `Diagnose()` - 20 edges

## Surprising Connections (you probably didn't know these)
- `run()` --calls--> `NewWithSource()`  [INFERRED]
...
Cohesion: 0.11
Nodes (61): ExitCode(), Model, T, TestInit(), asModel(), doneResults(), KeyMsg, Model (+53 more)

### Community 1 - "UI Model State"
Cohesion: 0.22
Nodes (10): Cmd, Model, listSSHKeys(), newSSHForm(), runSSH(), sshCommand(), sshHostValue(), Target (+2 more)

### Community 2 - "Probe Configuration Build"
Cohesion: 0.07
Nodes (58): CertPool, Config, Attempt, netops, Portal, Probe, ProbeID, ProbeResult (+50 more)
...
Cohesion: 0.12
Nodes (31): TestAppendJobLine(), TestClassifyJob(), TestConcurrentToolsCanSwitch(), TestDeferredQuit(), TestDeferredRestart(), TestDeferredRestartDefersTargetSwap(), TestEscClearsCommittedFilterBeforeClosing(), TestFilterLines() (+23 more)
...
### Community 10 - "ssid"
Cohesion: 0.06
Nodes (31): advertisedName, iwreq, F, avahiTXT(), AdvertisedNames(), Context, parseAvahiNames(), unescapeAvahi() (+23 more)
...
### Community 87 - "Target"
Cohesion: 0.15
Nodes (28): T, mustTarget(), TestBuildProbesNamesProtocolApplicationRow(), TestBuildProbesShape(), TestSSIDDoesNotGateNetworkProbes(), TestWrapRunTimesAndCleans(), Diagnose(), downgradeEgress() (+20 more)
...
Cohesion: 0.12
Nodes (23): FileMode, Cmd, Target, Tool, model, parseRunArgs(), copyReport(), exportReport() (+15 more)
...
### Community 109 - "IP"
Cohesion: 0.08
Nodes (42): Buffer, Proto, Target, FlagSet, IP, parsePort(), ParseTarget(), T (+34 more)
...
### Community 113 - "model"
Cohesion: 0.12
Nodes (13): CancelFunc, Context, Cmd, Model, Msg, Probe, ProbeID, ProbeResult (+5 more)
...
Cohesion: 0.10
Nodes (26): Context, proxyOps(), TestApplyDialWarnings(), TestBuildProbesProtoShapes(), TestCleanResultScrubsEveryTextField(), TestDialerFromUsesNetworkAddressType(), TestDialIPsEmpty(), TestDowngradeEgress() (+18 more)

### Community 130 - "ssh_test.go"
Cohesion: 0.15
Nodes (16): Cmd, must(), TestCapWriter(), TestSSHCommand(), TestSSHCommandNeedsHost(), TestSSHCommandWithoutHelper(), TestSSHFailureLandsInJobPane(), TestSSHFormClearsPasswordOnConnect() (+8 more)

### Community 131 - "scriptConn"
Cohesion: 0.32
Nodes (4): Builder, scriptConn, Time, Reader

### Community 132 - "fakeConn"
Cohesion: 0.33
Nodes (5): fakeConn, Addr, Conn, TestDialIPsRefused(), TestDialIPsSuccess()

### Community 135 - "jobState"
Cohesion: 0.18
Nodes (7): Cmd, model, job, JobStatus, Time, jobState, noticeDoneMsg

### Community 136 - "model.go"
Cohesion: 0.27
Nodes (9): loadHistory(), NewWithSource(), saveHistory(), lanNameMsg, lanNamesMsg, pendingAction, pendingKind, scheduleMsg (+1 more)

## Knowledge Gaps
- **204 isolated node(s):** `scheduleMsg`, `watchMsg`, `lanNamesMsg`, `lanNameMsg`, `sshDoneMsg` (+199 more)
...
- `Diagnose()` (7× useful, score=5.358919466)
- `README.md` (6× useful, score=4.139433979)
- `.handleViewKey()` (5× useful, score=3.601813874)
- `.outputView()` (5× useful, score=3.456776002)
- `.Update()` (4× useful, score=2.78438287) _(code changed — re-verify)_
- `JSON output` (3× useful, score=2.396541936)
- `.viewerFooter()` (3× useful, score=2.281715267)
- `report` (3× useful, score=2.124130583)
- `Install` (3× useful, score=1.722630327)
- `.networkHosts()` (2× useful, score=1.819704591)
...
- **Why does `NewWithSource()` connect `model.go` to `Model Extra Tests`, `UI Model State`, `Probe Configuration Build`, `Network Connection Test Doubles`, `IP`, `model`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `run()` connect `IP` to `Model Extra Tests`, `Probe Configuration Build`, `model.go`, `ssid`, `Target`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `Clean()` connect `ssid` to `IP`, `Job Execution Lifecycle`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Are the 17 inferred relationships involving `newModel()` (e.g. with `TestInit()` and `NewWithSource()`) actually correct?**
  _`newModel()` has 17 INFERRED edges - model-reasoned connections that need verification._
- **Are the 3 inferred relationships involving `asModel()` (e.g. with `TestCompletedRunSelectsFirstFailure()` and `TestDowngradeRunsWhenSkipsFinishRun()`) actually correct?**
  _`asModel()` has 3 INFERRED edges - model-reasoned connections that need verification._
- **Are the 28 inferred relationships involving `mustTarget()` (e.g. with `TestBatchedRunesReplayed()` and `TestEnterViewerBeforeOutput()`) actually correct?**
  _`mustTarget()` has 28 INFERRED edges - model-reasoned connections that need verification._
- **What connects `scheduleMsg`, `watchMsg`, `lanNamesMsg` to the rest of the system?**
