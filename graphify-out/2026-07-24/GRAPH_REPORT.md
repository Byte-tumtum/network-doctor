# Graph Report - network-doctor  (2026-07-24)

## Corpus Check
- 87 files · ~87,196 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 740 nodes · 1314 edges · 108 communities (59 shown, 49 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 166 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e95c02ef`
...
5. `netops` - 23 edges
6. `New()` - 20 edges
7. `mustTarget()` - 19 edges
8. `toolsFor()` - 19 edges
9. `ProbeResult` - 17 edges
10. `ProbeID` - 15 edges
...
## Communities (108 total, 49 thin omitted)

### Community 0 - "Model Extra Tests"
Cohesion: 0.11
Nodes (63): T, TestInit(), T, TestAppendJobLine(), TestClassifyJob(), TestConcurrentToolsCanSwitch(), TestDeferredQuit(), TestDeferredRestart() (+55 more)

### Community 1 - "UI Model State"
Cohesion: 0.06
Nodes (34): CancelFunc, Cmd, Context, Duration, appendJobLine(), depsState(), discoveredIPs(), filterLines() (+26 more)

### Community 2 - "Probe Configuration Build"
Cohesion: 0.10
Nodes (35): CertPool, Config, Attempt, netops, Probe, ProbeID, ProbeResult, Status (+27 more)
...
Cohesion: 0.08
Nodes (34): fakeConn, iwreq, scriptConn, Addr, Conn, Context, Reader, T (+26 more)

### Community 5 - "UI Application Toolbox"
Cohesion: 0.11
Nodes (39): ExitCode(), Model, T, mustTarget(), TestCompletedRunSelectsFirstFailure(), TestDowngradeRunsWhenSkipsFinishRun(), TestNADoesNotBlock(), TestSiblingIndependence() (+31 more)
...
Cohesion: 0.15
Nodes (23): FlagSet, buildReport(), Probe, ProbeID, ProbeResult, Target, init(), main() (+15 more)
...
### Community 20 - "model"
Cohesion: 0.15
Nodes (18): silentConn, T, Time, TestBannerProbeReadTimeout(), TestBannerProbeReadTimeoutHonorsContext(), TestDialIPsCancelledStopsEarly(), TestDialIPsRacesStaggered(), TestDNSProbeErrors() (+10 more)
...
### Community 87 - "Target"
Cohesion: 0.11
Nodes (24): Proto, Target, T, mustTarget(), TestBuildProbesNamesProtocolApplicationRow(), TestBuildProbesShape(), Diagnose(), T (+16 more)
...
## Knowledge Gaps
- **171 isolated node(s):** `scheduleMsg`, `lanNamesMsg`, `github.com/heymaikol/network-doctor`, `iwreq`, `ToolOutputMsg` (+166 more)
...
- `Diagnose()` (6× useful, score=5.201101928)
- `.outputView()` (5× useful, score=4.06031266) _(code changed — re-verify)_
- `README.md` (5× useful, score=3.764038711)
- `.Update()` (4× useful, score=3.270522883) _(code changed — re-verify)_
- `.handleViewKey()` (4× useful, score=3.214570583) _(code changed — re-verify)_
- `DowngradeEgress()` (3× useful, score=2.617856141)
- `report` (3× useful, score=2.494993686) _(code changed — re-verify)_
- `Install` (3× useful, score=2.023393394)
- `.View()` (2× useful, score=1.73759184) _(code changed — re-verify)_
- `JSON output` (2× useful, score=1.721507363)
...
- **Why does `New()` connect `model` to `Model Extra Tests`, `UI Model State`, `ExitCode`, `Network Connection Test Doubles`, `UI Application Toolbox`, `Probe Construction Tests`, `Target`?**
  _High betweenness centrality (0.177) - this node is a cross-community bridge._
- **Why does `newModel()` connect `Model Extra Tests` to `ExitCode`, `model`, `UI Application Toolbox`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `Clean()` connect `Probe Configuration Build` to `ssid`, `Network Connection Test Doubles`, `Job Execution Lifecycle`, `Target`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Are the 33 inferred relationships involving `newModel()` (e.g. with `TestInit()` and `TestConcurrentToolsCanSwitch()`) actually correct?**
  _`newModel()` has 33 INFERRED edges - model-reasoned connections that need verification._
- **Are the 14 inferred relationships involving `asModel()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`asModel()` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 5 inferred relationships involving `keyMsg()` (e.g. with `TestConcurrentToolsCanSwitch()` and `TestDeferredQuit()`) actually correct?**
  _`keyMsg()` has 5 INFERRED edges - model-reasoned connections that need verification._
- **What connects `scheduleMsg`, `lanNamesMsg`, `github.com/heymaikol/network-doctor` to the rest of the system?**
