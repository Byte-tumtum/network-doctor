...
type Service struct {
	...
	// DNSFault is a bounded, precomputed response schedule. It is consumed by
	// query type, never generated by a service goroutine.
...
// When a sequence is exhausted, the service answers normally.
...
func (n *Node) validateServices(names map[string]bool) error {
	for i := range n.Services {
		...
		case ServiceHTTP:
			...
			if svc.Certificate != nil || svc.DNSFault != nil || len(svc.Zone) != 0 || len(svc.Records) != 0 {
		...
		case ServiceTCP:
			...
			if svc.Certificate != nil || svc.DNSFault != nil || len(svc.Zone) != 0 || len(svc.Records) != 0 || svc.Status != 0 || svc.Body != "" {
