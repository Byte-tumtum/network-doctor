...
`T` opens the theme picker: up/down or `j`/`k` move through the built-in themes and apply the highlighted one at once, `enter` keeps it, and `esc` restores the theme that was active when the picker opened. It is presentation only, so no probe, diagnosis, status, report, snapshot, or exit code changes with it, and every status keeps the glyph and word it always had. The built-ins are `terminal` (the default, the 16 ANSI colours, which follow your terminal's own palette), `harbor`, `ember`, and `contrast`; the three alternates use adaptive colours rather than assuming a dark background, and `contrast` draws dim text at full strength instead of dimming it. There is no `--theme` flag: the picker is the way to choose one.
