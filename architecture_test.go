...
func TestPackageLayering(t *testing.T) {
	// Internal dependencies must descend this order. UI and simulation are
	// peers, so neither can depend on the other; checking every direct edge also
	...
	err := filepath.WalkDir("internal", func(path string, d fs.DirEntry, err error) error {
		...
		for _, spec := range f.Imports {
			...
			if sourceKnown && dependencyLayer >= sourceLayer {
				t.Errorf("%s: package layering violation: %s depends on %s; rule: dependencies must point down ui/simulation -> diagnostic -> textsafe", fset.Position(spec.Pos()), pkg, dependency)
			}
		}
		return nil
	})
